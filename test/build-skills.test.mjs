...
//     2. 두 빈자리의 분리(재사용 비결):
//          - 값-빈자리({test_command}) → 컴파일 때 본문에 *박힘* (golden 에 'npm test' 가 그대로 보임).
...
//            이게 "같은 Core, 다른 구현 주입"을 가능케 하는 새 모델의 핵심 — golden 이 회귀를 잡는다.
...
test("build-skills golden — 컴파일 SKILL.md 가 스냅샷과 일치 (값 박힘 vs 능력 런타임읽기)", () => {
  ...
  try {
    ...
    // 값-빈자리는 박힌다.
    assert.ok(testSkill.includes("npm test"), "값-빈자리 {test_command} 가 본문에 박히지 않음");
