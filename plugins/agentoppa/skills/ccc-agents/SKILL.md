...
2. **형식이 갈리는데 "한 벌"로 착각** — skills(`SKILL.md`)·hooks(`hooks.json`)와 달리 **양쪽이 읽는 공유 파일이 없다**: Claude `.claude/agents/<name>.md`(MD+frontmatter) ≠ Codex `.codex/agents/<name>.toml`(TOML), 디렉토리도 다름. → **단일 소스(.md) + 생성**으로 푼다(§형식 차이).
...
2. **단일 책임 + description** — 한 에이전트는 한 가지를 잘. `description`은 **"언제 위임/선택하나"를 front-load**(Claude 자동위임·Codex 선택 둘 다의 근거).
3. **능력 범위(`access`)** — 기본은 **read-only**. Claude는 `tools` allowlist, Codex는 `sandbox_mode`로 표현이 다르니 소스엔 **의도(`access: read-only|read-write`)**로 적는다. 쓰기 병렬은 충돌 주의(Claude는 `isolation: worktree`로 회피).
...
5. **검증 + 실제 스폰 테스트** — `validate.mjs`로 점검하고, 양쪽에서 실제로 스폰해 본다. 지시는 격리 컨텍스트에서도 자립하도록(self-contained).
