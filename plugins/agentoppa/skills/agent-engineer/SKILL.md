...
- **산출물 = SOURCE → COMPILED.** 유저가 손대는 `.harness/`(config + core + project) → AgentOppa가 `.claude/`·`.codex/`로 굽는다. 둘 다 git 커밋(생성물 독립).
...
### 4. 포장 — 양쪽 도구로
컴파일된 스킬·에이전트·훅을 `.claude/`·`.codex/`로 싣는다 → [ccc-plugin](../ccc-plugin/SKILL.md) (두 매니페스트·포인터·마켓 동기화).
