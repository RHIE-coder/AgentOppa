...
## 4. 컴파일 결과 — `.claude/skills/spec/SKILL.md` (슬롯 박힘 → AgentOppa-독립)
...
(에이전트 본체는 `project/agents/<name>.md`, 컴파일 → `.claude/agents/` + `.codex/*.toml`.)
...
**포인트:** phase 소스(`.harness/project/phases/`)는 그대로, **슬롯만 config·contract로 채워져** `.claude/`·`.codex/`로 컴파일된다. 다른 흐름이면 `config`의 `phases`만 바꾸면 끝.
