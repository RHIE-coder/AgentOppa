...
양 문서가 같은 말을 한다: **"무엇을 하는가"가 아니라 "언제 쓰는가"**, 핵심 사용사례·트리거 단어를 **앞에 배치**(front-load).
