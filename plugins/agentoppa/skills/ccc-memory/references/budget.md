...
degradation은 "컨텍스트에 들어갔냐"가 아니라 세 가지의 함수다. ARCHITECTURE.md를 읽는 *그 한 턴*만 보면 긴 AGENTS.md와 똑같이 나쁘지만, 셋을 다 보면 다르다:
...
- **지시:** 동시에 지킬수록 **준수율↓**(IFScale: 500개에서 68%, primacy bias로 뒤 지시 무시). 긴 지시파일은 토큰만 먹는 게 아니라 *중요한 규칙까지 안 지켜지게* 한다.
