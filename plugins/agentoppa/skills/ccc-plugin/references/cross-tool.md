...
| 마켓플레이스 | `.claude-plugin/marketplace.json` | `.agents/plugins/marketplace.json` (+legacy `.claude-plugin/`) |
...
- **최소:** 검증기로 드리프트를 막는다 — `validate.mjs`가 name 불일치=error, version/description 불일치=warn.
- **권장:** 생성 스크립트(`bin/build-manifests.mjs`)로 *공통 메타 + 컴포넌트 스캔 → 두 매니페스트 생성*. Codex쪽은 존재하는 컴포넌트에 포인터를 자동 부여, Claude쪽은 메타만. ccc-agents의 `build-agents.mjs`와 같은 빌드 브리지 철학(런타임 공유 파일이 없으니 빌드타임에 푼다).
...
- **교집합(skills/hooks/agents/MCP)은 한 트리에 공유, 매니페스트·마켓만 도구별 어댑터.** 이게 AgentOppa 전체를 관통하는 원칙이고, ccc-plugin은 그 *최외곽 래퍼*다.
