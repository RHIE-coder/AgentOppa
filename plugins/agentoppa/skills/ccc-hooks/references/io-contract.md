...
모든 hook은 stdin으로 JSON 한 덩어리를 받는다. 공통 필드:
...
| **2** | 차단 에러 | 아니오 | 이벤트별로 차단(아래 §3), stderr가 모델/사용자에 전달 |
| 1·3+ | 비차단 에러 | 아니오 | stderr 첫 줄 표시 후 계속 |

**두 가지 차단 방법:** ① exit 2 + stderr 메시지(간단), ② exit 0 + `hookSpecificOutput` JSON(정밀). 정밀 제어가 필요하면 ②.
