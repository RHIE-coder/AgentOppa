...
> 핵심 사실: **Codex의 10개 이벤트는 전부 Claude에도 있다(Codex ⊂ Claude).** 겹치는 이름은 철자까지 동일하다. 그래서 *이 10개 안에서, command 타입 + stdin JSON + exit2/hookSpecificOutput*만 쓰면 **hooks.json 한 벌로 양쪽 동작**한다. 벗어나는 순간(예: `Notification`, `type: prompt`) Claude 전용이 된다.
...
> matcher가 의미 있는 건 도구 이벤트(`PreToolUse`/`PostToolUse`/`PermissionRequest` → 도구명)와 일부 소스 이벤트(`SessionStart` → 소스)뿐. 안전한 이식형 matcher는 `Write|Edit` 같은 형태 — Claude·Codex 양쪽에서 같은 의미(상세: [`cross-tool.md`](cross-tool.md) §2).
