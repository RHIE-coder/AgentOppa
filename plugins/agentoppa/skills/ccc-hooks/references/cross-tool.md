...
- **stdin JSON 입력**, **exit 2 차단**, **`hookSpecificOutput` JSON 출력** — 계약 모양 동일([`io-contract.md`](io-contract.md)).
...
| **matcher** | 리터럴/파이프목록, 특수문자 있으면 정규식 | **항상 정규식**(`"^Bash$"`) |
...
| | Claude Code | Codex |
|---|---|---|
| 매니페스트 | `.claude-plugin/plugin.json` — `hooks/` 자동발견 | `.codex-plugin/plugin.json` |
...
5. **`notify`는 hook이 아니다** — stdin 아닌 argv, 하이픈 필드, 차단 불가, user-level `config.toml`의 테이블 섹션보다 **앞**에. 절대 hook과 섞지 말 것.
