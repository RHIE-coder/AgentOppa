#!/usr/bin/env node
// scaffold — 이 Core 를 소비하는 프로젝트의 .harness/config.yaml 을 깔거나, 빠진 능력 빈자리를 알려준다.
...
//   멱등: config 가 이미 있으면 *덮지 않고* 안 채워진 능력 빈자리만 보고한다(인터페이스 변경 = 자동 전파). zero-dep · 크로스OS.
...
function skeleton() {
  ...
  if (vals.length) {
    L.push("# 값 빈자리는 Core 빌드 때 저자 값이 이미 박혔다 — 이 프로젝트에서 바꾸려면 Core 재빌드(AgentOppa) 필요:");
    for (const v of vals) L.push("#   " + v.key + " (단계: " + v.phases.join(",") + ")");
  }
  return L.join("\n") + "\n";
}
...
if (!existsSync(cfgPath)) {
  ...
  if (caps.length) {
    ...
    console.log("능력 빈자리 없음 — 그대로 단계 스킬을 쓰면 된다.");
  }
  process.exit(0);
}

// 이미 있으면: 덮지 않고 안 채워진 능력 빈자리만 보고 — 인터페이스가 바뀌어 빈자리가 늘어도 여기서 잡힌다(전파).
const cur = readFileSync(cfgPath, "utf8");
const bound = new Set();
let inB = false;
for (const raw of cur.split(/\r?\n/)) {
  if (/^bindings:\s*(#.*)?$/.test(raw)) { inB = true; continue; }
  if (!inB) continue;
  if (/^\s+/.test(raw)) {
...
const missing = caps.filter((cp) => !cp.optional && !bound.has(cp.key));
if (missing.length) {
  console.log(".harness/config.yaml 있음 — 아직 안 채워진 능력 빈자리 " + missing.length + "개:");
  for (const cp of missing) console.log("  - " + cp.key + "  ← bindings 에 이 프로젝트의 구현을 적어라");
  process.exit(0);
}
console.log("✓ .harness/config.yaml 의 능력 빈자리가 모두 채워져 있다 — 그대로 쓰면 된다.");
process.exit(0);
