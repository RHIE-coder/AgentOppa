...
**검증:** intent 안 바꾸고 재생성하면 Core 묶음(`.agentoppa/`)이 그대로인가(스퓨리어스 diff 0). 하네스를 수십 번 재실행할 거라 핵심.
**fail:** 재생성이 `.agentoppa/` 를 바꿈.
**판정:** `compiled_idempotent` = 기계(순수 `git diff`).

## 절차
1. `setup`(커밋된 하네스 = baseline) → 2. `build-skills` 재실행 → 3. `judge`.
