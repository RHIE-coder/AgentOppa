...
```
*(Core 내부·`.harness/` 세부 배치는 `agent-engineer` 스킬과 `build-skills.mjs`에 구현돼 있다.)*
...
- **소비 프로젝트 자급(설치만 · AgentOppa 없이):** 빌드된 Core는 `interface.json`(빈자리 명세) + `setup` 스킬을 함께 싣는다. 남이 그 Core를 설치하면 `setup`이 `.harness/config.yaml`을 *스스로 깔고* 능력 빈자리를 노출 → 소비자는 `bindings`만 채운다. **AgentOppa는 Core를 *만들* 때만 필요(= 컴파일러).** Core가 바뀌어 빈자리가 늘면 `setup` 재실행이 빠진 자리를 알린다(= 자동 전파).
